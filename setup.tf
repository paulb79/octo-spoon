...
resource "aws_key_pair" "bdec_developer" {
  key_name      = "bdec-developer-key"
...
resource "aws_instance" "bdec" {
  ...
  key_name      = "${aws_key_pair.bdec_developer.id}"
